...
#     par(mfcol = c(2,1));acf(dt[,i]);pacf(dt[,i]);par(mfcol = c(1,1))
    ...
    tsdisplay(diff(dt[,i],1),main = finFeatList[i])
    dev.off()
...
for(i in 1:ncol(dt)){
    # jpeg(paste0('../Image/Lagged_Predictors/stationary1/',gsub("[^[:alnum:]]", "", finFeatList[i]),'_lag_cor.jpg'),width=600, height=400)
    jpeg(paste0('../Image/Lagged_Predictors/stationary2/',gsub("[^[:alnum:]]", "", finFeatList[i]),'_lag_cor.jpg'),width=600, height=400)
    x <- dt[,i]; y <- dt[,29]
    x <- (log(x) - mean(log(x))); y <- (log(y) - mean(log(y)))
    diff1x <- diff(x,1);diff1y <- diff(y,1)
    # pwx=diff1x;newpwy =diff1y
    # ar1model = auto.arima(x)#arima(x, order = c(1,0,0))
    # pwx = ar1model$residuals
    # ar2model = auto.arima(y)#arima(y, order = c(10,0,0))
    # newpwy=ar2model$residuals
    # filter(y, filter = c(1,-(1+ar1model$coef[1]),ar1model$coef[1]), sides =1)
    cffvalues <- ccf(pwx,newpwy,na.action=na.omit,lag.max=lags, plot=T,ylab = "cross-correlation",ylim=c(-1,1),
...
for(i in 1:ncol(dt)){
    ...
    x <- dt[,i]; y <- dt[,29]
    cffvalues <- ccf(x,y,na.action=na.omit,lag.max=lags, plot=T,ylab = "cross-correlation",ylim=c(-1,1),
                     main=paste0(finFeatList[i]," & New Vehicle Sales"))
    cffdf[,1] <- cffvalues$lag; cffdf[,(i+1)] <- cffvalues$acf
    dev.off()
}
...
Find_Max_CCF<- function(x,y, lags) { 
    diff1x <- diff(x,1)
    diff1y <- diff(y,1)
    # ar1model = auto.arima(diff1x)#arima(x, order = c(1,0,0))
    # pwx=ar1model$residuals
    # ar2model = auto.arima(y)
    # newpwy=ar2model$residuals
    # newpwy = filter(y, filter = c(1,-(1+ar1model$coef[1]),ar1model$coef[1]), sides =1)
    d <- ccf(diff1x,diff1y,na.action=na.omit,lag.max=lags, plot=T,ylab = "cross-correlation",ylim=c(-1,1),
             main=paste0(finFeatList[i]," & New Vehicle Sales"))
    upperCI <- qnorm((1 + 0.99)/2)/sqrt(d$n.used)
    lowerCI <- -qnorm((1 + 0.99)/2)/sqrt(d$n.used)
    cor = d$acf[,,1] 
    lag = d$lag[,,1] 
...
########################
# source('pairs.R')
# pairs(dt[,sample(1:29,5)],lower.panel=panel.smooth,upper.panel=panel.acf,diag.panel=panel.hist) 
# pairs(dt[,sample(1:29,5)],upper.panel = panel.acf, diag.panel = panel.acf, cex.labels = 1)

# jpeg(paste0('../Image/pair_lag.jpg'),width=2000, height=2000)
# par(mfcol=c(29,29))
# for(i in 1:ncol(dt)){
#     x <- dt[,i]; y <- dt[,29]
#     diff1x <- diff(x,1)
...
#     pwx=diff1x
#     newpwy =diff1y
#     cffvalues <- ccf(pwx,newpwy,na.action=na.omit,lag.max=12, plot=T,ylab=NULL,ylim=c(-1,1),
#                      main=paste0('v',i, '&', 'v',i))
# }
# dev.off()
# 
# library(rgl)
# i <- 10; j <- 15
# x <- dt[,i]; y <- dt[,j]; z <- dt[,29]
# diff1x <- diff(x,1)
# diff1y <- diff(y,1)
# diff1z <- diff(z,1)
# plot3d(diff1x,diff1y,diff1z,col = rainbow(1000), size=3)
# 
# barchart(ccf(diff1x,diff1y,plot=F)$acf)
