...
for(i in 1:ncol(dt)){
    ...
    diff1x <- diff(x,1)
    diff1y <- diff(y,1)
    # ar1model = arima(x, order = c(1,0,0))#auto.arima(x)
    pwx=diff1x#ar1model$residuals
    # ar2model = arima(y, order = c(10,0,0))#auto.arima(y)
    # newpwy=ar2model$residuals
    newpwy =diff1y# filter(y, filter = c(1,-(1+ar1model$coef[1]),ar1model$coef[1]), sides =1)
